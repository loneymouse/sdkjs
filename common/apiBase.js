...
(function (window, undefined) {
	...
	baseEditorsApi.prototype._init = function () {
		...
		var t = this;
		var innerCallFunc = function (msg) {
			if (msg.type == 'message') {
				let innerdata = JSON.parse(msg.data);
				if (innerdata) {
					if (innerdata.type == 'Custom') {
						let command = t[innerdata.command]
						if (command) {
							command.call(t, innerdata.data)
						}
					}
				}
			}
		...
		if (window.attachEvent) {
			window.attachEvent('onmessage', innerCallFunc);
		}
		else {
			window.addEventListener('message', innerCallFunc, false);
		}
	...
	baseEditorsApi.prototype.asc_loadFontsFromServer = function (fonts) {
		if (!fonts) {
			fonts = ["Arial", "Symbol", "Wingdings", "Courier New", "Times New Roman", "Cambria Math"];
		}
