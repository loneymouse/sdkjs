...
CDocument.prototype.AddParaMathLatex = function (ntype, nLatex) {
	if ((c_oAscMathType.Default_Text === ntype) && (!this.IsSelectionUse() || !this.IsTextSelectionUse())) {
		let textPr = this.GetDirectTextPr();
		let mathm = new AscCommonWord.MathMenu(c_oAscMathType.Default_Text, textPr);
		mathm.SetText(nLatex);
		this.AddToParagraph(mathm);
		this.MoveCursorRight(false, 1, false)
	}
};
