			...
			if (0 === type) {
				...
				if (_current["Url"] !== undefined) {
					...
					this.api.insertDocumentUrlsData = {
						imageMap: null, documents: [{ url: _current["Url"], format: _current["Format"], token: _current["Token"] }], convertCallback: function (_api, url) {
...
background-repeat: no-repeat;\
	...
	asc_docs_api.prototype.asc_AddMath_Latex = function (nlatex) {
		var oLogicDocument = this.private_GetLogicDocument();
		if (!oLogicDocument)
			return;
		...
		if (!oLogicDocument.IsSelectionLocked(AscCommon.changestype_Paragraph_Content)) {
			oLogicDocument.StartAction(AscDFH.historydescription_Document_AddMath);
			oLogicDocument.AddParaMathLatex(c_oAscMathType.Default_Text, nlatex);
			oLogicDocument.FinalizeAction();
		}
	};
	...
	asc_docs_api.prototype.asc_DocumentData = function () {
		let oLogicDocument = this.private_GetLogicDocument();
		if (!oLogicDocument)
			return []
		const rs = []
		var arrParagraphs = []
		oLogicDocument.GetAllParagraphs({ All: true }, arrParagraphs);
		arrParagraphs.forEach((parap, index) => {
			let nparag = {
				index: index,
				ranges: []
			}
	...
	asc_docs_api.prototype['asc_AddMath_Latex'] = asc_docs_api.prototype.asc_AddMath_Latex;
